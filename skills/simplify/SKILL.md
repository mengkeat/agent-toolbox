...
## Phase 2: Launch Three Review Agents in Parallel if supported by the harness, otherwise run sequentially

Launch all three agents concurrently (if supported by harness) in a single message. Pass each agent the full diff so it has the complete context.
